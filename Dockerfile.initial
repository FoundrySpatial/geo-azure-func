...
FROM mcr.microsoft.com/azure-functions/python:4-python3.8 as builder
# RUN apt-get makecache fast
RUN apt-get -y update
RUN apt-get install -y build-essential git autoconf libtool flex bison cmake make tar gzip automake libpng-dev nasm libxml2-dev libreadline-dev curl cmake openssl
...
# # JpegXL
# ENV JXL_VERSION=0.6.1
# RUN mkdir /tmp/jxl \
#   && curl -sfL https://github.com/libjxl/libjxl/archive/refs/tags/v0.6.1.tar.gz | tar zxf - -C /tmp/jxl --strip-components=1 \
#   && cd /tmp/jxl \
#   && mkdir build && cd build \
...
# "ada3e517416596430872656e74e2c99648e03402" is the latest commit on release/3.5 branch

# gdal
RUN mkdir /tmp/gdal \
  && curl -sfL https://github.com/OSGeo/gdal/archive/ada3e517416596430872656e74e2c99648e03402.tar.gz | tar zxf - -C /tmp/gdal --strip-components=1 \
  && cd /tmp/gdal \
  && mkdir build && cd build \
  && cmake .. \
    -DCMAKE_BUILD_TYPE=Release \
...
FROM mcr.microsoft.com/azure-functions/python:4-python3.8 as runner
